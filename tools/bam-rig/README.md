...
the bracket with ≥ 4 walls.

**Sliced 2026-09-24** on the print host with `ender5s1_petg_fast.ini` (PrusaSlicer 2.9.6),
as two plates because the CLI cannot mix infill on one plate and `--merge` segfaults in
that build — the plates were arranged here (`build/plate1-bracket-arms.stl`,
`build/plate2-pots-lids.stl`, regenerated by hand with trimesh, not by `rig.py`) and sliced
as single models centred on the measured mesh (104, 123):

| G-code (staged in `~/bam-rig` on the print host, not in Mainsail) | Parts | Infill | Filament | Estimate |
|---|---|---|---|---|
| `bam-rig-plate1-bracket-arms-100pc` | bracket, arm_100, arm_150 | 100 % rectilinear | 125.6 g | 6 h 15 m |
| `bam-rig-plate2-pots-lids` | 2 × pot, 2 × lid | profile 15 % grid | 107.7 g | 4 h 54 m |

Both footprints sit inside the probed mesh with margin; no supports. The STLs and the slicer failures seen on the way (`--merge` segfault in the Flatpak 2.9.6 CLI; the Debian 2.5 binary's locale error; no output when writing into `~/printer_data/gcodes`) are staged in `~/bam-rig/NOTES.md` on the print host for the owner's printer session to diagnose and slice. All seven parts would
fit one plate only by touching the mesh edge, and one plate cannot carry two infills.
The bracket at 100 % is a choice: it carries the load reaction and the extra ~40 g costs
nothing that matters. PETG is less stiff than PLA+, and the arm's droop under load is
