...
| Cameras | 1–2 | For LeRobot recording (DEC-08). None owned. Upstream's standard is a **32 × 32 mm UVC camera module** (innomaker-type, manual focus, run at 640 × 480 / 30 fps) — one on the wrist, optionally one overhead; the printed mounts are in upstream `Optional/` ([`hardware.md`](hardware.md)) |
