...
|---|---|---|---|
| **Mapping and navigation** (hexapod, tank): a metric range scan for SLAM and Nav2 costmaps | A 2D lidar gives a 360° laserscan directly, which is what Nav2 consumes; the hexapod's depth camera only ever made a fake laserscan from one 10-pixel band | Nothing (checked stock and invoices 2026-09-23: only the aircraft's single-point TFmini Plus and a Matek optical-flow unit) | LD19 / STL-19P-class DTOF lidar ~£50–80; **Slamtec RPLIDAR C1** (datasheet v1.1, read 2026-09-23): DTOF, 0.05–12 m (6 m on 10 %-reflective black), 5 kHz, 8–12 Hz, 0.72° at 10 Hz, ±30 mm, 110 g, Ø55.6 × 41.3 mm, 5 V / 230 mA typical, UART 460800 via its USB adapter, Class 1; per-point "reflectivity" is a quality byte (LaserScan `intensities`), not a second depth axis. ROS 2 Jazzy: `rplidar_ros` 2.1.4 is released from apt and `sllidar_ros2` has a C1 launch; one known driver fix (range_min hard-coded at 0.15 m). **£57 UK on 2026-09-23 (owner), against £109 for an LDROBOT LD19** — the C1 is the pick. Arguably better value for this task than any depth camera |
